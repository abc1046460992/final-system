            ...
            try:
                ...
                if os.path.exists(dist_exe):
                    ...
                    try:
                        ...
                        self.after(0, lambda: messagebox.showwarning("تنبيه", f"التطبيق ({exe_name}.exe) قيد التشغيل. يرجى إغلاقه أولاً حتى نتمكن من تحديثه."))
                        return
                
                # 5. Copy Data Files Folder
                src_db = os.path.join(os.getcwd(), "قاعدة_البيانات")
                dst_db = os.path.join(delivery_dir, "قاعدة_البيانات")
                if os.path.exists(src_db):
                    from shutil import copytree
                    EXCLUDE = ['معلم', 'المعلم', 'انتظار', 'اشراف', 'تبادل']
                    def custom_ignore(dir, files):
                        return [f for f in files if any(x in f for x in EXCLUDE)]
                    try:
                        copytree(src_db, dst_db, dirs_exist_ok=True, ignore=custom_ignore)
